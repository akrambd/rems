...
<?php
	...
	$sql = $conn->prepare("SELECT property_request.*,user_info.*,property_info.* FROM property_request 
						   ...
						   WHERE property_request.receiver_id='$user_id' AND property_info.active_status_property='1' AND property_request.active_status_request='pending' ORDER BY property_request.request_id DESC");
	...
	$data = $sql->fetchAll();
		...
		$sql = $conn->prepare("SELECT property_request.*,user_info.*,property_info.* FROM property_request 
						   		LEFT JOIN user_info ON property_request.sender_id=user_info.user_id 
						   		INNER JOIN property_info ON property_request.property_id=property_info.id  
						   WHERE property_request.receiver_id='$user_id' AND property_request.active_status_request='pending' ORDER BY property_request.request_id DESC");
